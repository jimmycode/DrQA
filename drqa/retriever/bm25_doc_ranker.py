...
def get_bm25_matrix(cnts, Nt, dls, k1, b):
    ...
    * k1, b: BM25 parameters
    ...
    nom = cnts * (k1 + 1.)
    binary = (cnts > 0).astype(int)  # to avoid dense matrix in the following line
    denom = cnts + k1 * binary.multiply(B)
    tfs = nom.multiply(denom.power(-1))  # sparse shape [hash_size, nb_docs]
...
class BM25DocRanker(object):
    ...
    def __init__(self, count_path, k1=1.2, b=0.75, strict=True):
        """
        Args:
        ...
        count_matrix, metadata = utils.load_sparse_csr(count_path)
        doc_freqs = metadata['doc_freqs']
        doc_lens = metadata['doc_lens']

        logger.info('Making BM25 matrix...')
        self.doc_mat = get_bm25_matrix(count_matrix, doc_freqs, doc_lens, k1, b)
        del count_matrix  # to save memory
